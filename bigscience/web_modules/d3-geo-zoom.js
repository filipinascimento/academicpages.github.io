import { d as dispatch, t as timer, l as timeout, i as interpolateNumber, g as color, h as interpolateRgb, j as interpolateString, m as interpolateTransformSvg, o as interpolateTransformCss, p as now, q as cubicInOut, v as interpolateZoom } from './common/cubic-c714ef8b.js';
...
var geoZoom = index({
  ...
  init: function init(nodeEl, state) {
    select(nodeEl).call(state.zoom = zoom().scaleExtent(state.scaleExtent).on('start', zoomStarted).on('zoom', zoomed));
    var v0, r0, q0;
    ...
    function getPointerCoords(zoomEv) {
      ...
      return pointers$1 && pointers$1.length > 1 ? [0, 1].map(function (idx) {
        return avg(pointers$1.map(function (t) {
          return t[idx];
        }));
      }) // calc centroid of all points if multi-touch
      : pointers$1.length ? pointers$1[0] // single point click
      : [undefined, undefined];
    }
  }
});
